...
- **`release/`** — the build/pack/publish pipeline for this package itself (`build-bin.ts`, `build-postinstall.ts`, `prepare-release-package.ts`, `release.ts`, `publish-package.ts`, `run-tools.ts`). Only the maintainer runs this; it's never executed by a consumer and isn't part of what gets published. `publish-package.ts` (`package:publish`) bumps the version for real via `npm version` (real commit + tag, not a throwaway suffix), builds/packs through the existing `package:pack` pipeline, then force-pushes the resulting `.tgz` — committed by itself — to the orphan `pacote-compilado` branch, recreated from scratch each run in a temp worktree under `.worktrees/`. Publishing to consumer projects is a separate, manual step — not something this script does.
...
npm run package:publish -- [patch|minor|major]  # bumps version for real (npm version), packs, force-pushes the .tgz to the orphan `pacote-compilado` branch
```
